import { kbDir, queryMemoryAndKB, wrapForPrompt, type RetrievedChunk } from "../qmd";
import { composeRetrievedEvidence } from "../editorial-composition";
import { deriveScope, narrowToRequestedSection } from "../marker-scope";
import { chat, SOCRATIC_SYSTEM_PROMPT, EDU_GUIDE_RULE, type ChatMessage } from "../llm";
import { checkResponse, deterministicResponseFlags, rewriteSocratic } from "../guardrail";
import { logAuditEventSafe } from "../audit";
import { localize } from "../i18n";
import { canRecoverMissingCitation, hasVisibleAssistantText, prepareAssistantText } from "../assistant-text";
import { responseTokenBudget } from "../conversation-policy";
import { resolveRetrievalQuery } from "../query-rewrite";
import { getLabSeriesSet } from "../memory-reader";
import { buildLabSeriesAnswer } from "../lab-chat-fallback";
import { displayMarker, selectInlineLabSeries } from "../health-map";
...
export async function runCurrentChat(context: ChatExecutionContext): Promise<ChatRunResult> {
  ...
  try {
    ...
    return { status: 500, body: { error: "retrieval_failed" } };
  }
