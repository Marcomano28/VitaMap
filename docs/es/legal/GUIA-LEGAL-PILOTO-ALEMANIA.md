...
> **Nota 2026-08-02 — figura del acompanante (supporter).** El punto "acceso de
> medicos, terapeutas, investigadores o familiares" tiene ya diseno tecnico
> implementado y desactivado por flag (ADR-018), mas dos borradores pendientes
> de revision juridica, cada uno en espanol (trabajo) y aleman (destinado a ser
> el vinculante): `ESTATUTOS-SUPPORTER.md` / `de/STATUTEN-BEGLEITPERSON.md` y
> `CONSENTIMIENTO-PERSONA-ACOMPANADA.md` /
> `de/EINWILLIGUNG-BEGLEITETE-PERSON.md`. La decision abierta principal es
> **quien es el responsable del tratamiento** cuando un acompanante introduce
> datos de una persona que no tiene cuenta (§0.1 de los estatutos). Nada de
> esto entra en el piloto sin abrir formalmente esta Etapa D.
